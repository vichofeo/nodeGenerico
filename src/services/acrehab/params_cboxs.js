...
const PDEPENDENCIES = {    
    eval_box:{        
        ...
        ilogic: {
            ...
            ORDER BY 2`,
            institucion_id: `SELECT DISTINCT i.institucion_id AS value, d.nombre_dpto ||' - '||  i.nombre_institucion text
            FROM  ae_institucion i, al_departamento d, ae_institucion eg
            WHERE i.tipo_institucion_id='EESS'
            AND i.cod_dpto =  d.cod_dpto
            ...
            ORDER BY 2`,
            dni_evaluador: `SELECT p.dni_persona AS VALUE, p.primer_apellido || ' ' || p.segundo_apellido ||' '|| p.nombres AS text
            FROM aep_institucion_personal ip, au_persona p
            WHERE ip.dni_persona = p.dni_persona AND ip.institucion_id = '$inst'
            ORDER BY 2`
        },
        referer: [        
        ],
