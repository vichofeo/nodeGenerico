...
const COMBOSSNIS = {
    ...
    ['snis.gestion', 'snis.gestion'], ['snis.mes', "to_char(snis.mes,'00')"],
    ['snis.formulario', 'snis.formulario'], ['snis.grupo', 'snis.grupo']],
    labels: ['Ente Gestor', 'Establecimiento de salud', 'Gestion', 'Mes', 'Formulario', 'Grupo'],
    name_cbo: ['eg', 'eess', 'gestion', 'mes', 'frm', 'grp'],
    default_cbo: [true, true, false, true, false, false]
}
...
const fsnisReportParams = async (dto) => {
    try {
        ...
        for (let control = 0; control < selects.length; control++) {
            ...
            if(COMBOSSNIS.default_cbo[control] && aux.length>0)
            aux.unshift(initial)
